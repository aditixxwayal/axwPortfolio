...
function BlackIconWrapper({ icon, ...rest }) {
  return (
    <IconWrapper
      icon={icon}
      bgColor='bg-[#D3A0D1]'  // Adjusted to a soft teal for better contrast with indigo
      textColor='text-[#D86D9C]'  // Darker text for better readability
      {...rest}
    />
  );
}
...
function RedIconWrapper({ icon, ...rest }) {
  return (
    <IconWrapper
      icon={icon}
      bgColor='bg-[#FAD1D1]'  // Warm reddish tone to complement the indigo background
      textColor='text-[#FF8B8B]'  // White text for visibility
      {...rest}
    />
  );
}
...
function IconWrapper({ icon, bgColor, textColor, target, isEmail }) {
  return (
    <a
      ...
      className={`w-20 h-20 md:w-14 md:h-14 lg:w-20 lg:h-20 ${bgColor} ${textColor} flex items-center justify-center text-3xl transition-all hover:text-4xl cursor-pointer border border-black`}
    >
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-3 md:w-1/2">
        {/* Pass the icons to the respective wrappers */}
        <BlackIconWrapper icon={<FaEnvelope />} target="mailto:wayaladiti27@gmail.com" />
        <RedIconWrapper icon={<FaLinkedin />} target="https://www.linkedin.com/in/aditiwayal/" />
        <BlackIconWrapper icon={<FaGithub />} target="https://github.com/aditixxwayal" />
        <BlackIconWrapper icon={<FaInstagram />} target="https://instagram.com/aditixwayal" />
        <RedIconWrapper icon={<FaPhoneAlt />} target="mob:+917020739495" />
      </div>
    </>
  );
}
